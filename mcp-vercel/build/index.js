...
function getToken() {
    ...
    if (!t) {
        throw new Error("VERCEL_TOKEN or VERCEL_ACCESS_TOKEN is required. Create one at https://vercel.com/account/tokens");
    }
    return t;
}
...
// --- list_projects
server.tool("list_projects", "List all Vercel projects for the authenticated user or team", {
...
// --- list_deployments
server.tool("list_deployments", "List deployments, optionally filtered by projectId, target (production/preview), or state", {
    ...
    try {
        ...
        const text = JSON.stringify({ deployments: list, pagination: data.pagination }, null, 2);
        return { content: [{ type: "text", text }] };
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
...
// --- get_deployment
server.tool("get_deployment", "Get deployment details by ID or URL (e.g. dpl_xxx or xxx.vercel.app)", {
...
server.tool("get_deployment_events", "Get build logs/events for a deployment. Use builds=1 and limit to get recent logs.", {
    ...
    try {
        const params = { builds: String(builds) };
        if (limit != null)
            params.limit = String(limit);
        if (direction)
...
// --- get_project
server.tool("get_project", "Get a single project by ID or name", {
